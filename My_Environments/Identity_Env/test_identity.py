...
from stable_baselines import A2C, ACER, ACKTR, DDPG, DQN, PPO2, PPO1, TRPO
from stable_baselines.ddpg import AdaptiveParamNoiseSpec
from stable_baselines.common.vec_env import DummyVecEnv
from stable_baselines.common import set_global_seeds
from identity_env import *
...
@pytest.mark.slow
@pytest.mark.parameterize("model_name", ['a2c', 'acer', 'acktr', 'dqn', 'ppo1', 'ppo2', 'trpo'])
def test_identity(model_name):
    env = DummyVecEnv([lambda: IdentityEnv(10)])
    model = LEANR_FUNV_DICT[model_name](env)
    ...
    for _ in range(n_trials):
        ...
        obs, reward, _, _ = env.step(actions=action)
        reward_sum += reward
    assert model.action_probability(obs).shape == (1, 10), "Error: action_probability not of correct shape"
    assert np.prod(model.action_probability(obs, actions=env.action_space.sample()).shape) == 1, "Error: not scalar " \
                                                                                                 "probability"
