...
class vaegan(object):
    # build model
    def __init__(self, batch_size, max_iters, repeat, model_path, data_ob, latent_dim, sample_path, log_dir,
                 learnrate_init):
    ...
    def build_model_vaegan(self):
        ...
        self.D_real_loss = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.ones_like(self.D_pro_logits) - d_scale_factor,
        ...
        self.G_fake_loss = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.ones_like(self.G_pro_logits) - g_scale_factor,
                                                    logits=self.G_pro_logits))
        self.G_tilde_loss = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.ones_like(self.De_pro_tilde) - g_scale_factor,
                                                    logits=self.De_pro_tilde))
            ...
            while step <= self.max_iters:
                ...
                if step % 200 == 0:
                    D_loss, fake_loss, encode_loss, LL_loss, kl_loss, new_learn_rate \
                        = sess.run([self.D_loss, self.G_loss, self.encode_loss, self.LL_loss,
                                    self.kl_loss / (self.latent_dim * self.batch_size), new_learning_rate],
                                   feed_dict=fd)
                    print("Step %d: D: loss = %.7f G: loss=%.7f E: loss=%.7f LL loss=%.7f KL=%.7f, LR=%.7f" % (
                    step, D_loss, fake_loss, encode_loss, LL_loss, kl_loss, new_learn_rate))
                ...
                if np.mod(step, 200) == 1:
                    ...
                    save_images(sample_images[0:self.batch_size], [self.batch_size / 8, 8],
    ...
    def test(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            save_images(real_images[0:self.batch_size], [self.batch_size / 8, 8],
    ...
    def generate(self, z_var, reuse=False):
        ...
        with tf.variable_scope('generator') as scope:
            ...
            d1 = tf.nn.relu(
                batch_normal(fully_connect(z_var, output_size=8 * 8 * 256, scope='gen_fully1'), scope='gen_bn1',
                             reuse=reuse))
            d2 = tf.reshape(d1, [self.batch_size, 8, 8, 256])
            d2 = tf.nn.relu(batch_normal(de_conv(d2, output_shape=[self.batch_size, 16, 16, 256], name='gen_deconv2'),
                                         scope='gen_bn2', reuse=reuse))
            d3 = tf.nn.relu(batch_normal(de_conv(d2, output_shape=[self.batch_size, 32, 32, 128], name='gen_deconv3'),
                                         scope='gen_bn3', reuse=reuse))
            d4 = tf.nn.relu(batch_normal(de_conv(d3, output_shape=[self.batch_size, 64, 64, 32], name='gen_deconv4'),
